...
| `fig_6.py` | `source_data/figure_6/csv/` (+ `radiologist_df.csv`) | 5 inputs: `cv_predictions_min.csv`, `model_case_confidence.csv`, `pair_level_metrics.json`, `salary_progression.csv`, `gpu_training_metadata.json`, plus the canonical `radiologist_df.csv` from `figure_1/csv_v2/`. Individual radiologist performance, confidence calibration (Q3/Q1 quartile split), equivalent-experience regression, and NHS salary integration are all computed live via `_metrics_utils.compute_individual_perf` / `compute_confidence_analysis` / `compute_equiv` / `compute_financial`. `model_case_confidence.csv` holds one scalar per case, `top_percentile_prob`, the mean of the voxels at or above the 99th percentile of that case's enhancing-tumour probability map; this is the model-alone confidence used in the calibration panels and in Table 1's calibration rows. The `figure_6_rng_state.json` file (~13 KB, in `source_data/figure_6/`, not its `csv/` subdirectory) restores the NumPy RNG state so the scatter jitter is byte-identical across runs. |
