...
| Point estimates (radiologist accuracy family) | Per-reader value on the reader's **raw binary prediction** (`_metrics_utils.compute_group_metrics`), averaged over the 11 readers |
| Point estimates (radiologist sens/spec) | Per-reader value on the **confidence-direction-adjusted score thresholded at 0.5** (`_per_reader_metrics`, `table_1.py:166`) — the binary rating of the same score the ROC/PR curves use, under which an answer given with confidence below 5 is treated as favouring the opposite class. Because this is a different decision rule from the raw prediction, balanced accuracy and (sensitivity + specificity)/2 are **not** equal in Table 1 (0.698 vs 0.656 without support; 0.743 vs 0.730 with). |
...
- **Inter-rater agreement (Cohen's κ)** row of Table 1 (row 10) is sourced from `supplementary_figure_4.py` (Supplementary Figure 4; case-level bootstrap, resampling cases with replacement and recomputing κ over every pair belonging to the resampled cases). `table_1.py` itself produces rows 1–9 (accuracy family + confidence-accuracy correlation + calibration difference), row 11 (mean confidence) and row 12 (RTAT) using paired reader-level / pair-level / case-level bootstraps at B=5,000, seed=20260505. See the "Bootstrap procedures used in `table_1.py`" table above for the sampling unit used per row.
